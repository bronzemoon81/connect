...
(function (dom) {
  ...
  async function show(dropdown) {
    ...
    let dropdownContent = dom(dropdownBox).find(".dropdown-content").first();
    ...
    if (dom(dropdownContent).length) {
      ...
      dom(dropdown).css("position") == "static"
        ...
        : "";
      dom(dropdownContent).css("width", dom(dropdownContent).css("width"));
      ...
      // Init popper
      Popper.createPopper(dropdownToggle[0], dropdownContent[0], {
        placement: placement,
        modifiers: [
          {
            name: 'offset',
  ...
  dom("body").on("click", function (event) {
    ...
    let dropdownBox = dom(dropdown).find(".dropdown-content").first();
    let activeDropdownBox = dom(event.target).closest(".dropdown-content").first();
